"""PySide6 Demo for ExpertAnything Knowledge UI.

Demonstrates modern UI with cards, stats dashboard, and interactive elements.
Loads real data from ExpertAnything core system.
"""
import json
import math
import os
...
from expert_anything.core.models import KnowledgeAsset, Concept, Relation
...
class KnowledgeGraphView(QGraphicsView):
    ...
    def set_concepts_and_relations(self, concepts, relations, learner_state):
        """Populate the graph with concepts and relations."""
        scene = self.scene()
        scene.clear()
        ...
        for i, (cid, cdata) in enumerate(concepts.items()):
            angle = (2 * math.pi * i / len(concepts)) - (math.pi / 2)
            x = center.x() + radius * math.cos(angle)
            y = center.y() + radius * math.sin(angle)
            positions[cid] = QPointF(x, y)
        ...
        for r in relations:
            ...
            if src_id in positions and tgt_id in positions:
                line = QGraphicsLineItem(
                    positions[src_id].x(), positions[src_id].y(),
                    positions[tgt_id].x(), positions[tgt_id].y()
                )
                line.setPen(pen)
                scene.addItem(line)
        ...
        for cid, cdata in concepts.items():
            ...
            mastery = self._get_mastery(name, learner_state)
            ...
            ellipse = QGraphicsEllipseItem(-25, -25, 50, 50)
            ...
            text = QGraphicsTextItem(name[:15] + ('...' if len(name) > 15 else ''))
            text.setDefaultTextColor(QColor(50, 50, 50))
            text.setPos(pos.x() - 30, pos.y() + 30)
            scene.addItem(text)

        self.fitInView(scene.sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)

    def _get_mastery(self, name, learner_state):
        from expert_anything.core.learner import normalize
        key = normalize(name)
        rec = learner_state.get('concepts', {}).get(key, {})
        return rec.get('mastery', 0.0)

    def mousePressEvent(self, event):
        item = self.itemAt(event.pos())
        if isinstance(item, QGraphicsEllipseItem):
            concept_name = item.data(0)
            if concept_name:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ExpertAnything · PySide6 Demo")
    ...
    def _build_concept_map_view(self):
        """Build the concept map view showing the knowledge graph."""
        widget = QWidget()
        layout = QVBoxLayout(widget)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(0)
        ...
        if self.current_asset_id and self.current_asset_id in self.assets:
            ...
            # Info label
            info_label = QLabel(f"共 {len(concepts)} 个概念，{len(relations)} 条关系。滚轮缩放，拖拽平移，点击节点开始学习。")
            ...
            """)
            self._graph_view.setMinimumHeight(400)
            content_layout.addWidget(self._graph_view)

            # Build concept data for graph
            concept_dict = {}
            for c in concepts:
                norm_name = normalize(c.get('name', ''))
                concept_dict[c.get('id')] = {
                    'name': c.get('name', ''),
                    'norm': norm_name,
                    'mastery': self.learner.get('concepts', {}).get(norm_name, {}).get('mastery', 0)
                }

            self._graph_view.set_concepts_and_relations(concept_dict, relations, self.learner)

            # Legend
            legend_label = QLabel("图例：绿色(✓ 已掌握) 橙色(⚠ 学习中) 红色(○ 未学习)")
