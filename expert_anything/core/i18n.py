...
_STRINGS: dict[str, dict[str, str]] = {
    ...
    "app_error": {"zh-CN": "程序错误", "en": "Application error", "ja": "アプリケーションエラー"},
    "app_error_body": {
        "zh-CN": "发生未处理的错误：\n{exc}\n\n详情已写入 error.log（{log}）。您可以继续使用，但建议反馈此问题。",
        "en": "An unhandled error occurred:\n{exc}\n\nDetails were written to error.log ({log}). You may continue, but please report this.",
        "ja": "未処理のエラーが発生しました:\n{exc}\n\n詳細は error.log（{log}）に記録されました。続行できますが、報告をお勧めします。",
    },
    "stat_total": {"zh-CN": "总概念", "en": "Concepts", "ja": "総概念"},
    "stat_unstudied": {"zh-CN": "未学习", "en": "Unstudied", "ja": "未学習"},
    "stat_anomaly": {"zh-CN": "异常", "en": "Anomalies", "ja": "疑義"},
    "tab_concepts": {"zh-CN": "概念掌握度", "en": "Concepts", "ja": "概念の習得度"},
    "tab_history": {"zh-CN": "学习历史", "en": "History", "ja": "履歴"},
    "dash_subtitle": {
        "zh-CN": "来源：{title} | 概念数：{concepts} | 方法：{method}",
        "en": "Source: {title} | Concepts: {concepts} | Method: {method}",
        "ja": "出典: {title} | 概念: {concepts} | 方法: {method}",
    },
    "dash_loading": {"zh-CN": "加载知识数据...", "en": "Loading knowledge data…", "ja": "知識データを読み込み中…"},
    "tag_cross": {"zh-CN": "跨资产", "en": "Cross-asset", "ja": "資産横断"},
    "group_unknown": {"zh-CN": "未知资产", "en": "Unknown asset", "ja": "不明な資産"},
    "group_other": {"zh-CN": "其他来源", "en": "Other sources", "ja": "その他の出典"},
    "share_concept": {"zh-CN": "共享概念", "en": "shared", "ja": "共有概念"},
    "edge_prereq": {"zh-CN": "前置", "en": "prerequisite", "ja": "前提"},
    "edge_related": {"zh-CN": "关联", "en": "related", "ja": "関連"},
    "global_map_title": {"zh-CN": "全局知识图谱", "en": "Global Knowledge Map", "ja": "グローバル知識マップ"},
    "hint": {"zh-CN": "提示", "en": "Notice", "ja": "通知"},
    "error": {"zh-CN": "错误", "en": "Error", "ja": "エラー"},
    "done": {"zh-CN": "完成", "en": "Done", "ja": "完了"},
    "teach_error": {"zh-CN": "教学失败: {msg}", "en": "Teaching failed: {msg}", "ja": "学習の生成に失敗: {msg}"},
    "followup_failed": {"zh-CN": "（追问失败：{msg}）", "en": "(follow-up failed: {msg})", "ja": "（質問に失敗: {msg}）"},
    "score_line": {"zh-CN": "得分: {s:.2f}", "en": "Score: {s:.2f}", "ja": "スコア: {s:.2f}"},
    "feedback_line": {"zh-CN": "反馈: {fb}", "en": "Feedback: {fb}", "ja": "フィードバック: {fb}"},
    "start_teach_first": {"zh-CN": "请先开始教学会话", "en": "Start a lesson first", "ja": "先にレッスンを開始してください"},
    "paste_placeholder": {"zh-CN": "在这里粘贴你的学习材料...", "en": "Paste your learning material here…", "ja": "ここに学習教材を貼り付け…"},
    "llm_ready": {"zh-CN": "✓ LLM 已配置", "en": "✓ LLM configured", "ja": "✓ LLM 設定済み"},
    "llm_not_ready": {"zh-CN": "✗ LLM 未配置（使用确定性回退）", "en": "✗ No LLM (deterministic fallback)", "ja": "✗ LLM未設定（決定論的フォールバック）"},
    "filter_supported": {"zh-CN": "支持的文件", "en": "Supported files", "ja": "対応ファイル"},
    "teacher_empty": {
        "zh-CN": "教师模型数据尚未生成\n请先完成知识资产导入和自学习过程",
        "en": "Teacher model not generated yet\nImport an asset and run the self-learning pass first",
        "ja": "教師モデルはまだ生成されていません\n先に資産のインポートと自己学習を実行してください",
    },
    "teacher_build_now": {"zh-CN": "立即生成教师模型", "en": "Generate now", "ja": "今すぐ生成"},
    "rechecking": {"zh-CN": "正在重新自检，请稍候...", "en": "Re-checking…", "ja": "再チェック中…"},
    "recheck_failed": {"zh-CN": "自检失败: {e}", "en": "Re-check failed: {e}", "ja": "再チェックに失敗: {e}"},
    "recheck_done": {"zh-CN": "自检完成: {n} 条待解项", "en": "Re-check done: {n} open anomalies", "ja": "再チェック完了: 未解決 {n} 件"},
    "build_needs_llm": {"zh-CN": "需要配置 LLM 才能生成教师模型", "en": "An LLM key is required to build the teacher model", "ja": "教師モデルの生成にはLLMキーが必要です"},
    "building_teacher": {"zh-CN": "正在生成教师模型，请稍候...", "en": "Building the teacher model…", "ja": "教師モデルを生成中…"},
    "build_failed": {"zh-CN": "生成失败: {e}", "en": "Build failed: {e}", "ja": "生成に失敗: {e}"},
    "build_done": {"zh-CN": "教师模型生成完成: {n} 条待解项", "en": "Teacher model built: {n} open anomalies", "ja": "教師モデル生成完了: 未解決 {n} 件"},
    "report_title": {"zh-CN": "ExpertAnything 学习报告", "en": "ExpertAnything Learning Report", "ja": "ExpertAnything 学習レポート"},
    "report_generated": {"zh-CN": "生成时间: {t}", "en": "Generated: {t}", "ja": "生成: {t}"},
    "report_assets": {"zh-CN": "【知识资产】", "en": "[Knowledge Assets]", "ja": "【知識資産】"},
    "report_concepts": {"zh-CN": "概念数: {c}, 关系数: {r}", "en": "Concepts: {c}, Relations: {r}", "ja": "概念: {c}, 関係: {r}"},
    "report_mastery": {"zh-CN": "【概念掌握度】", "en": "[Concept Mastery]", "ja": "【概念の習得度】"},
    "report_sources": {"zh-CN": "来源: {s}", "en": "Sources: {s}", "ja": "出典: {s}"},
    "report_history": {"zh-CN": "【最近评估记录】", "en": "[Recent Evaluations]", "ja": "【最近の評価記録】"},
    "report_score_fb": {"zh-CN": "得分: {s:.2f} | 反馈: {fb}", "en": "Score: {s:.2f} | Feedback: {fb}", "ja": "スコア: {s:.2f} | フィードバック: {fb}"},
    "report_done": {"zh-CN": "报告生成完毕", "en": "Report generated", "ja": "レポート生成完了"},
    "save_report": {"zh-CN": "保存学习报告", "en": "Save learning report", "ja": "学習レポートを保存"},
    "report_filename": {"zh-CN": "学习报告_{ts}.txt", "en": "report_{ts}.txt", "ja": "レポート_{ts}.txt"},
    "report_saved": {"zh-CN": "学习报告已保存到:\n{filename}", "en": "Report saved to:\n{filename}", "ja": "レポートを保存しました:\n{filename}"},
    "report_failed": {"zh-CN": "导出失败: {e}", "en": "Export failed: {e}", "ja": "エクスポートに失敗: {e}"},
    "priority_fmt": {"zh-CN": "优先级 {s:.2f}", "en": "priority {s:.2f}", "ja": "優先度 {s:.2f}"},
    "focus_title": {"zh-CN": "聚焦：{name}（概念结构图）", "en": "Focus: {name} (concept map)", "ja": "フォーカス: {name}（概念図）"},
    "concept_structure": {"zh-CN": "概念结构图", "en": "Concept Map", "ja": "概念図"},
    "import_success_title": {"zh-CN": "导入成功", "en": "Import succeeded", "ja": "インポート成功"},
    "import_failed": {"zh-CN": "处理失败: {msg}", "en": "Processing failed: {msg}", "ja": "処理に失敗: {msg}"},
    "selflearning": {"zh-CN": "正在自我学习...", "en": "Self-learning…", "ja": "自己学習中…"},
    "selflearning_done": {"zh-CN": "自我学习完成: {n} 条待解项", "en": "Self-learning done: {n} open anomalies", "ja": "自己学習完了: 未解決 {n} 件"},
    "asset_label": {"zh-CN": "知识资产", "en": "Knowledge Assets", "ja": "ナレッジ資産"},
